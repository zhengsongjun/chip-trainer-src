...
function calculateDrawmaha49Score(cards) {
  const rankValues = {
    '2': 2,
    '3': 3,
    '4': 4,
  ...
  return Math.abs(totalScore - 49);
}

function getBestHand(holeCards, board, gameMode) {
  if (gameMode === 'drawmaha-49') {
...
// 计算 Hand 分数（49点）
const player1HandScore = calculateDrawmaha49Score(testCase1.player1);
const player2HandScore = calculateDrawmaha49Score(testCase1.player2);

console.log("=== Hand 分数 ===");
console.log(`玩家 1: 分数 = ${player1HandScore} (距离 49)`);
console.log(`玩家 2: 分数 = ${player2HandScore} (距离 49)`);
console.log(`Hand 赢家: ${player1HandScore < player2HandScore ? '玩家 1' : '玩家 2'}`);
console.log();

// 计算 Board 牌力（Omaha High）
const player1Best = getBestHand(testCase1.player1, testCase1.board, 'drawmaha-49');
const player2Best = getBestHand(testCase1.player2, testCase1.board, 'drawmaha-49');
